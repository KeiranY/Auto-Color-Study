...
fn process_tcp_file() -> Option<c_int> {
    let mut buf = String::new();
    let res = unsafe {
        let path = CString::new("/proc/net/tcp").unwrap();
        let mode = CString::new("r").unwrap();
        let file_ptr = redhook::real!(fopen)(path.as_ptr(), mode.as_ptr());
        if file_ptr.is_null() {
            return None;
        }
...
fn handle_open(cpath: *const c_char, oflag: c_int, real_open: unsafe extern "C" fn(*const c_char, c_int) -> c_int) -> c_int {
    ...
    if path != c"/proc/net/tcp" {
        return unsafe { real_open(cpath, oflag) };
    }
...
fn handle_openat(
    ...
    cpath: *const c_char,
    oflag: c_int,
    real_openat: unsafe extern "C" fn(c_int, *const c_char, c_int) -> c_int,
) -> c_int {
    let path = unsafe { CStr::from_ptr(cpath) };
    ...
    let resolved_path = if path.to_bytes().starts_with(b"/") {
        ...
        if len == -1 {
            ...
            return unsafe { real_openat(dirfd, cpath, oflag) };
        }
...
fn handle_fopen(cpath: *const c_char, mode: *const c_char, real_fopen: unsafe extern "C" fn(*const c_char, *const c_char) -> *mut libc::FILE) -> *mut libc::FILE {
    ...
    if path != c"/proc/net/tcp" {
        return unsafe { real_fopen(cpath, mode) };
    }
    let fd = process_tcp_file();
    if let Some(fd) = fd {
        return unsafe { libc::fdopen(fd, mode) };
    }
    unsafe { real_fopen(cpath, mode) }
}
